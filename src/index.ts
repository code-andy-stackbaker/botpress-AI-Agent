...
const Ok = z.object({ result: z.literal('success'), conversion_rate: z.number() })
...
type Logger = { forBot(): { info(msg: string): void; error(msg: string): void } }
  ...
  actions: {
    async getCurrencyRate({
      ...
      if (!apiKey) {
        ...
        throw new (await import('@botpress/sdk')).RuntimeError('Missing EXCHANGE_RATE_API_KEY')
      }
      ...
      logger.forBot().info(`[exchange-rate] Fetching ${base}->${target} via ${url}`)

      let json: unknown
      try {
        const res = await fetch(url)
        if (!res.ok) {
          logger.forBot().error(`[exchange-rate] HTTP ${res.status} from upstream`)
          throw new Error(`HTTP ${res.status}`)
        }
        json = await res.json()
      } catch (e: any) {
        logger.forBot().error(`[exchange-rate] Network error: ${String(e)}`)
        throw new (await import('@botpress/sdk')).RuntimeError('Failed to reach ExchangeRate API')
      }

      const parsed = ApiResponse.safeParse(json)
      if (!parsed.success) {
        logger.forBot().error(`[exchange-rate] Unexpected API shape: ${JSON.stringify(parsed.error.issues)}`)
        throw new (await import('@botpress/sdk')).RuntimeError('Unexpected response from ExchangeRate API')
      }

      const data = parsed.data
      if (data.result === 'error') {
        logger.forBot().error(`[exchange-rate] API error: ${data['error-type']}`)
        throw new (await import('@botpress/sdk')).RuntimeError(`ExchangeRate API error: ${data['error-type']}`)
      }

      logger.forBot().info(`[exchange-rate] Rate ${base}->${target} = ${data.conversion_rate}`)
      return { rate: data.conversion_rate }
    },
  },

  channels: {},   // leave empty unless you implement a message channel
  handler: async () => { /* no-op: not using inbound webhooks/channels */ },
})
